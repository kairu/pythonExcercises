...
replace = re.findall("dr_type=\"(.*?)\"",textReg)
# replace2 = re.findall("<dic_ref .*?>|</dic_ref>",textReg)
replace3 = re.findall("dr_type=\".*?\">(.*?)<",textReg)
for tags,value in zip(replace,replace3):
    print("-> <{}>{}<\{}>".format(tags,value,tags))
